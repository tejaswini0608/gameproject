...
int main(void)         //main function
{
	 ...
	 for(i=0;i<a;i++)      //for loop is used to assign a slot type to a player randomly.a is the maximum number of players inputted by the user
	{
		   ...
		   if(j==1)           //if the random number generated is 1 then that player is assigned to ground
		   {
			...
			//printf("\n%s\n",players[i].ground);
			strcpy(players[i].slots,"ground");    //player from the struct slot is assigned ground slot
			printf("\nplayer %d is assigned to %s\n",i+1,players[i].slots); //player from struct data is printed
			players[i].slotsnum=j;    //player.slotsnum from struct data is taking the value of j
				  arraynumber[i]=i;
		   ...
		   if(j==2)
		   {
			 ...
			 // printf("\n%s\n",players[i].hill);
			  strcpy(players[i].slots,"hill");  //player from the struct slot is assigned hill
			printf("\nplayer %d is assigned to %s\n",i+1,players[i].slots); //player from struct data is printed
			  players[i].slotsnum=j;  //player.slotsnum from struct data is taking the value of j
			  arraynumber[i]=i;
		   ...
		   // printf("\n%s\n",players[i].city);
			strcpy(players[i].slots,"city");  //player from the struct slot is assigned city
			printf("\n player %d is assigned to %s\n",i+1,players[i].slots);//player from struct data is printed
			 players[i].slotsnum=j;    //player.slotsnum from struct data is taking the value of j
			 arraynumber[i]=i;
	...
	int number;
		for(l=0;l<a;l++)       //for loop where a is the total number of players.This loop asks each user to give specific input-move or attack
		{
		   ...
		   case 0:
		   {
			   if(players[l].slotsnum==1)  //if the player is assigned ground,that is the slotsnum is 1,then it moves forward to hill
			   {
				   strcpy(players[l].slots,"hill");
				   printf("\nplayer %d is assigned to %s\n",l+1,players[l].slots);
			   ...
			   else if(players[l].slotsnum==2)  //if the player is assigned hill initially,that is the slotsnum is 2,then it moves forward to city
			   { 
				   strcpy(players[l].slots,"city");
				   printf("\n player %d is assigned to %s\n",l+1,players[l].slots);
			   ...
			   else if(players[l].slotsnum==3)  //if the player is assigned city initially,that is the slotsnum is 3,then it cannot move forward 
			   {
					strcpy(players[l].slots,"city");

				   printf("\nplayer %d is assigned to %s\n and the player cannot move forward",l+1,players[l].slots);
			 ...
			 case 1:                       
			 {
				  if(players[l].slotsnum==1)    //if the player is assigned ground,that is the slotsnum is 1,so he cannot move backward
					  { 
							strcpy(players[l].slots,"ground");
							printf("\n player %d is assigned to %s\n and the player cannot move backwards",l+1,players[l].slots);
					   }
					 
					 
				 else if(players[l].slotsnum==2)// if the player is assigned to hill,that is slotnum=2 ,then he moves backwards to ground
						{
					   strcpy(players[l].slots,"ground");
					   printf("\nplayer %d is assigned to %s\n",l+1,players[l].slots);
				 ...
				 else if(players[l].slotsnum==3)// if the player is assigned to city ,that is slotnum=3 ,then he moves backwards to hill
						 {
				   strcpy(players[l].slots,"hill");
				   printf("\nplayer %d is assigned to %s\n",l+1,players[l].slots);
			 ...
			 case 2:                    //an attempt to work out the attack
			 {
					            //to know which player is nearest..the array prints out all the slots assisgned to the players
				 ...
				 int y=num;
				 for(x=0;x<a;x++)       //l is the number of player we are refering to the number of player
				 {
					 if(player[x].Strength<=70)
					 {
						player[l].Lifepoints=player[l].Lifepoints-0.5*player[l].Strength;
					 }
					 
					 else 
					 {
						 player[l].Lifepoints=player[l].Lifepoints-0.3*player[x].Strength;
